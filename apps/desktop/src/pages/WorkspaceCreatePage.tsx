...
import { ElevatedPanel, HintPanel, InsetPanel, PanelCard, PanelCardContent, PanelCardHeader } from "@/components/pagePanels";
...
function getEffectiveRepoOwner(
  ...
  if (!hasExplicitRepositoryBinding({
    name: "",
    slug: "",
    description: "",
...
function canSubmitWorkspaceCreateForm(
    ...
    <main className={withinShell ? "flex flex-col gap-6" : "app-frame min-h-screen p-6"}>
      <PanelSlotsLayout
        bodyClassName={
          withinShell
            ? undefined
            : "mx-auto min-h-[calc(100vh-3rem)] max-w-6xl"
        }
            ...
            >
                    ...
                    <form.Subscribe selector={(state) => getNormalizedSlug(state.values)}>
                      {(normalizedSlug) => (
                        <HintPanel>
                          저장 값:
                          {" "}
                          <span className="font-medium text-[var(--foreground)]">{normalizedSlug || "-"}</span>
                        </HintPanel>
                      )}
                    </form.Subscribe>
                  </div>
                    ...
                    <div className="grid gap-2.5 sm:col-span-2">
                      ...
                      <form.Subscribe
                        selector={(state) => ({
                          ...
                          effectiveRepoOwner: getEffectiveRepoOwner(state.values, defaultRepoOwner),
                        })}
                      >
                        {({ explicitRepositoryBinding, normalizedRepoName, effectiveRepoOwner }) => (
                          <div className="grid gap-2 text-xs text-[var(--muted-foreground)]">
                            <HintPanel>
                              저장 값:
                              {" "}
                              <span className="font-medium text-[var(--foreground)]">{normalizedRepoName || "-"}</span>
                            </HintPanel>
                            {!explicitRepositoryBinding ? (
                              <HintPanel>
                                비워두면 API 기본값이 적용됩니다:
                                ...
                                <span className="font-medium text-[var(--foreground)]">
                                  {(effectiveRepoOwner || defaultRepoOwner || "viewer-login")}/{normalizedRepoName || "workspace-docs"}
                                </span>
                              </HintPanel>
                            ) : null}
                          </div>
              ...
              <InsetPanel padding="md">
                <div className="grid gap-2 sm:grid-cols-2">
                  <form.Subscribe selector={(state) => canSubmitWorkspaceCreateForm(state.values, defaultRepoOwner)}>
                    {(canSubmit) => (
                      <PrimaryPageAction clientLog="워크스페이스 만들기" disabled={!canSubmit || isSubmitting} type="submit">
                        {isSubmitting ? <LoaderCircle className="animate-spin" /> : <ArrowRight />}
                        워크스페이스 만들기
                      </PrimaryPageAction>
                    )}
                  </form.Subscribe>
                  <SecondaryPageAction clientLog="워크스페이스 생성 취소" onClick={onCancel} type="button">
                    돌아가기
                  </SecondaryPageAction>
                </div>
              </InsetPanel>
