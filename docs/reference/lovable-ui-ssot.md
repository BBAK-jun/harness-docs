...
- 로그인 이후 진입한 화면은 온보딩 상태라도 앱 내부 컨텍스트를 유지한다.
- 즉 `워크스페이스 선택`, `워크스페이스 생성`, `초대 수락` 같은 화면도 가능한 한 LNB를 포함한 app shell 안에서 렌더링한다.
- 이유는 사용자가 온보딩 도중에도 다른 액션으로 이동할 수 있어야 하기 때문이다.
- 로그인 이후의 온보딩은 `인증 전 랜딩 화면`이 아니라 `인증 후 앱 내부 전환 상태`로 취급한다.
